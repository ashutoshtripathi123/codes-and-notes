...
public class CopyOfBricksAndBuildingPuzzle2 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        ArrayList<Integer>oddBuilding = new ArrayList<Integer>();
        ArrayList<Integer>evenBuilding = new ArrayList<Integer>();
		...
		for(int i=0;i<numberOfBuildings;i++)
		{
			...
			if(tempInput%2==0)
			{
				evenBuilding.add(tempInput);
			}
			else
			{
				oddBuilding.add(tempInput);
			}
		}
		...
		while(k<numberOfQueries)
		{
			...
			if(query[k]%2==0)
			{
				count = evenBuilding.size();
			}
			else
			{	
				while(j<numberOfBuildings)
				{
					if((oddBuilding.get(j)%query[k])==0)
					{
						++count;
						++j;
					}
