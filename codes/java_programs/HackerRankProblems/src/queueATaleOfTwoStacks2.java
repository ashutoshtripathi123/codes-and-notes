...
problem statement above. All three queries start with an integer denoting the query , 
...
public class queueATaleOfTwoStacks2 {
    public static class MyQueue<T> {
        Stack<T> stackNewestOnTop = new Stack<T>();
        Stack<T> stackOldestOnTop = new Stack<T>();

        /*Check if stack O is empty or not. If not then empty all the elements
         * from stack O to stack N, and then perform the new enqueue operation*/
        public void enqueue(T value)
        { 
            stackNewestOnTop.push(value);
        }

        /*Check if stack O is empty or not. If not then print the top.
         * If the stack O is empty, however N is not, then move all elements from N to O*/
        public T peek()
        {
            prepOld();
        ...
        /*Check if stack O is empty or not. If not then pop the top element. 
         * If stack is empty, then move all elements from N to O and then perform a pop*/
        public void dequeue() 
        {
            prepOld();
